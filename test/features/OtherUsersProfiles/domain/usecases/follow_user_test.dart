...
void main() {
  ...
  late OtherUserFollowUsecase otherUserFollowUsecase;
  setUp(() {
    mockOtherUserProfilesRepository = MockOtherUserProfilesRepository();
    otherUserFollowUsecase =
        OtherUserFollowUsecase(mockOtherUserProfilesRepository);
  });
  group('test other user follow usecase', () {
    test('follow user usecae should return failure on Left when error occurs',
        () async {
      // arrange
      ...
      // act
      final result = (await otherUserFollowUsecase(tFollowUnfollowUserParams))
          .fold((l) => l, (r) => null);
      // assert
      expect(result, isA<Failure>());
      verify(() => mockOtherUserProfilesRepository
          .followUser(tFollowUnfollowUserParams)).called(1);
    });
  });
}
